...
public class Supermarche extends Batiment {
	...
	private static final Dimension3D[] DIMENSIONS = {new Dimension3D(64,62,20), new Dimension3D(64,62,40)};
	...
	private static final String[] IMAGES = {Configuration.COMPLETE_BATIMENTS_DIR+"Supermarche.gif", "Supermarche-1"};

	private int niveau=0;

	public Supermarche(Position position) {
		super("Commerce",
				"Supermarché",
				position,
				DIMENSIONS[0],
	...
	public boolean ameliorer() {
		...
		if(isAmeliorationPossible()) {
			...
			this.setImagePath(Supermarche.IMAGES[niveau_suivant]);
			//Actualisation du niveau
			this.setNiveau(niveau_suivant);

			if(Configuration.MODE=="debug")
				System.out.println("SUPERMARCHE : Amélioration du supermarché à la position "+this.getPosition().toString()+"."); 

			return true;
		} else {
			if(Configuration.MODE=="debug")
				System.out.println("SUPERMARCHE : Niveau maximum déjà atteint.");
	...
	private void update_amelioration_possible(int niveau_suivant) {
		if(isAmeliorationPossible()) {
			if(niveau_suivant <= Supermarche.NIVEAU_MAX) is_amelioration_possible = true;
