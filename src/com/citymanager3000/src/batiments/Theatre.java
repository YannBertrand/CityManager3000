...
public class Theatre extends Batiment {
	...
	private static final String[] IMAGES = {Configuration.COMPLETE_BATIMENTS_DIR+"Theatre.gif", "Theatre-1"};

	private int niveau=0;

	public Theatre(Position position) {
	...
	public boolean ameliorer() {
		...
		if(isAmeliorationPossible()) {
			...
			this.setImagePath(Theatre.IMAGES[niveau_suivant]);
			//Actualisation du niveau
			this.setNiveau(niveau_suivant);

			if(Configuration.MODE=="debug")
				System.out.println("THEATRE : Amélioration du théatre à la position "+this.getPosition().toString()+"."); 

			return true;
		} else {
			if(Configuration.MODE=="debug")
				System.out.println("THEATRE : Niveau maximum déjà atteint.");
	...
	private void update_amelioration_possible(int niveau_suivant) {
		if(isAmeliorationPossible()) {
			if(niveau_suivant <= Theatre.NIVEAU_MAX) is_amelioration_possible = true;
