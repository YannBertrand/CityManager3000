 ...
 *
 */
public class CentreCommercial extends Batiment {
	private static final int NIVEAU_MAX = 1;

	private static final Dimension3D[] DIMENSIONS = {new Dimension3D(120,120,20), new Dimension3D(120,120,40)};

	private static final double PRIX_CONSTRUCTION = 25000000;
	private static final double[] BENEFICE_PAR_MOIS = {750000,2000000};
	private static final double[] PRIX_AMELIORATIONS = {40000000};
	private static final double[] PRIX_DESTRUCTIONS = {5000000,10000000};

	private static final String[] IMAGES = {Configuration.COMPLETE_BATIMENTS_DIR+"CentreCommercial.gif", "CentreCommercial-1"};

	private int niveau=0;

	public CentreCommercial(Position position) {
		super("Commerce",
				"Centre Commercial",
				position,
				DIMENSIONS[0],
	...
	public boolean ameliorer() {
		...
		if(isAmeliorationPossible()) {
			...
			this.setImagePath(CentreCommercial.IMAGES[niveau_suivant]);
			//Actualisation du niveau
			this.setNiveau(niveau_suivant);

			if(Configuration.MODE=="debug")
				System.out.println("CENTRE COMMERCIAL : Amélioration du centre commercial à la position "+this.getPosition().toString()+"."); 

			return true;
		} else {
			if(Configuration.MODE=="debug")
				System.out.println("CENTRE COMMERCIAL : Niveau maximum déjà atteint.");
	...
	private void update_amelioration_possible(int niveau_suivant) {
		if(isAmeliorationPossible()) {
			if(niveau_suivant <= CentreCommercial.NIVEAU_MAX) setIsAmeliorationPossible(true);
